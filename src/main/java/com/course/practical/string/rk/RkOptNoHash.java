package com.course.practical.string.rk;
...
public class RkOptNoHash {
    ...
    private int calFirstSubStrHashCode(String substring) {
        ...
        int hashCode = 0;
        for (int i = 0; i < n; i++) {
            hashCode += (int) Math.pow(26, i) * (substring.charAt(n - i - 1) - 'a');
        }
    ...
    private int calHashCode(String mainString, int i, int[] hashCodes, int n) {
        return hashCodes[i - 1] * 26 - (mainString.charAt(i - 1) - 'a') * (int) Math.pow(26, n) + (mainString.charAt(i + n - 1) - 'a');
    }

    public static void main(String[] args) {
        RkOptNoHash b = new RkOptNoHash();
